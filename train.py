...
modelPath =  './resultModel/V1_1210/'
EncFileName = "Enc.pth"
DecFileName = "Dec.pth"
writer = SummaryWriter(log_dir="./logs/V1_1210")
...
for epoch in range(num_epochs):
    ...
    for oriCSIBatch,labelCSIBatch in dataloader_train:
        ...
        #start training
        optimizer.zero_grad()
        code = Enc(oriCSIBatch)
        reconCSIBatch = Dec(code)
        ...
        lossSumBatchs = lossSumBatchs + loss
    ...
    with torch.no_grad():
        ...
        for oriCSITestBatch,labelCSITestBatch in dataloader_test:
            ...
            #testing
            codeTest = Enc(oriCSITestBatch)
            reconCSITestBatch = Dec(codeTest)
            ...
            lossSumTestBatchs = lossSumTestBatchs + lossTest
    ...
    if (epoch > 2) & (loss_test_mat[epoch,0]<loss_test_mat[epoch-1,0]):
        torch.save(Enc.state_dict(),os.path.join(modelPath,EncFileName))
        torch.save(Dec.state_dict(),os.path.join(modelPath,DecFileName))
