...
Dec = Decoder().to(device)
    ...
    #load from history
Enc.load_state_dict(torch.load(os.path.join(modelPath,EncFileName),map_location=torch.device('cpu')))
Dec.load_state_dict(torch.load(os.path.join(modelPath,DecFileName),map_location=torch.device('cpu')))
